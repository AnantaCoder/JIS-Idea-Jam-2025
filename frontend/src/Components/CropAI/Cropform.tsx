...
export default function CropForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post(
        '/api/crop-prediction/',  // Now using Vite's proxy
        apiData,
        {
          headers: {
      ...
      setResult(response.data.prediction);// Update result state
    } catch (err) {
      // Handle errors
  ...
  return (
    <section id="crop-form" className="py-20 min-h-screen px-4">
          ...
          <div className="md:flex-1 p-6 md:p-8 items-center bg-gray-700">
            ...
            {loading ? (
              ...
              <p className="text-gray-400">
                Enter your land details to get a crop recommendation.
              </p>
            )}
          </div>
        </div>
